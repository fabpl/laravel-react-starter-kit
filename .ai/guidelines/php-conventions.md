...
- Use constructor property promotion: `public function __construct(private GitHub $github) {}`.
- Prefer PHPDoc array shapes (`array<int, string>`, `array{value: string, label: string}`)
  ...
  (`@return HasMany<Post, $this>`, `/** @use HasFactory<\Database\Factories\UserFactory> */`).
...
- **Guaranteed relations:** when a related model is an invariant, expose an
  explicit `*OrFail()` accessor using `firstOrFail()` rather than silencing the
  nullable type.
