...
- Use model **factories** and their states; do not hand-build models.
...
- Target elements by their `data-test` attribute via the `@name` selector
  (e.g. `@login-button`). Add `data-test` to UI you need to drive.
...
- Drive **server-side** validation by submitting input that passes the browser's
  ...
  — an empty submit is blocked by the browser first.
