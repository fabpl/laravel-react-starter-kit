...
A hardened starter kit built on the official Laravel React stack, with a quality
toolchain wired in from day one. Everything below is optimised for a fast,
predictable **developer experience**: one command to set up, one command to run,
...
| Tests        | Pest — Unit · Feature · **Browser (Playwright)**, ≥ 90% coverage     |
...
| Run one test file/filter            | `php artisan test --filter=Authentication` |
...
4. **Pest** — the full suite (Unit + Feature + Browser) with a **≥ 90% coverage gate**
...
```
app/
  Actions/        Single-purpose actions (Fortify actions)
  Concerns/       Reusable traits (validation rule sets)
